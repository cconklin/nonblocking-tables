...
void bm_func(nonblocking::lock_free_dictionary& dict, int ops, int reads, int writes, int deletes, std::atomic<bool>& start)
{
    while (!start.load());
...
double benchmark(int t, int ops, int reads, int writes, int deletes)
{
    ...
    nonblocking::lock_free_dictionary obmap(32768);
